(** Library for managing commandline arguments and for printing
    ...
    Users of the library will specify possible commandline options
    using the add* functions, which each is given a name for the
    commandline option, a default value for the option, and optional
    documentation for the commandline option.
    ...
    usage or version information is printed and the process exists.
*)
...
fun addX (string_to_v: string -> 'v option, v_to_string: 'v -> string) (s:string, v:'v, docopt:doc option) : 'v getter =
...
val addInt : string * int * doc option -> int getter = addX (string_to_int, int_to_string)
val addReal : string * real * doc option -> real getter = addX (string_to_real, real_to_string)
val addBool : string * bool * doc option -> bool getter = addX (string_to_bool, bool_to_string)
val addString : string * string * doc option -> string getter =
    addX (fn s => SOME s,
          fn s => s)
...
fun rightPad n s =
    if size s <= n then rightPad n (s ^ " ")
    else s

fun leftPad n s =
    if size s <= n then leftPad n (" " ^ s)
    else s

fun printUsageExit ecode =
    let val n = CommandLine.name()
        val v = #2(!version)
        val d = List.foldl (fn ((s,doc,NONE),d) =>
                               ("-" ^ s :: map (indent 2) doc @ d)
                             | ((s,doc,SOME def),d) =>
                               (rightPad 39 ("-" ^ s) ^ " " ^ leftPad 38 ("(" ^ def ^ ")") :: map (indent 2) doc @ d))
                           []
                           (!options)
        val d =
            ["Usage: " ^ n ^ " " ^ #2(!usage),
